...
export function SessionCard({ session, trainers, virtualTrainers, topics, sessionTypes, canEdit, isToday, attendanceSummary, onEdit, onDelete, onAttendance }: Props) {
  ...
  return (
    ...
    >
      ...
      {/* Probetraining badge */}
      {!cancelled && (session.probetraining_count ?? 0) > 0 && (
        <span className="inline-flex items-center gap-1 text-[10px] font-semibold text-amber-600 dark:text-amber-400 mb-1.5">
          <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
          ...
          Probetraining · {session.probetraining_count}
        </span>
      )}
